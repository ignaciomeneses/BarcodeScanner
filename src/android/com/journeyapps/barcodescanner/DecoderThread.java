...
public class DecoderThread {
    ...
    private final Handler.Callback callback = new Handler.Callback() {
        ...
        public boolean handleMessage(Message message) {
            if (message.what == mFakeR.getId("id","zxing_decode")) {
    ...
    public DecoderThread(Context context, CameraInstance cameraInstance, Decoder decoder, Handler resultHandler) {
    ...
    private final PreviewCallback previewCallback = new PreviewCallback() {
        ...
        public void onPreview(SourceData sourceData) {
            ...
            synchronized (LOCK) {
                if (running) {
                    // Post to our thread.
                    handler.obtainMessage(mFakeR.getId("id","zxing_decode"), sourceData).sendToTarget();
                }
            }
        }
    };
    ...
    private void decode(SourceData sourceData) {
        ...
        if (rawResult != null) {
            ...
            if (resultHandler != null) {
                ...
                Message message = Message.obtain(resultHandler, mFakeR.getId("id","zxing_decode_succeeded"), barcodeResult);
            ...
            if (resultHandler != null) {
                Message message = Message.obtain(resultHandler, mFakeR.getId("id","zxing_decode_failed"));
                message.sendToTarget();
            }
        }
        if (resultHandler != null) {
            List<ResultPoint> resultPoints = decoder.getPossibleResultPoints();
            Message message = Message.obtain(resultHandler,mFakeR.getId("id","zxing_possible_result_points"), resultPoints);
            message.sendToTarget();
        }
        requestNextPreview();
    }
