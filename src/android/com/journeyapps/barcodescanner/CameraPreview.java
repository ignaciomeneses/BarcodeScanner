...
public class CameraPreview extends ViewGroup {
    ...
    private final Handler.Callback stateCallback = new Handler.Callback() {
        ...
        public boolean handleMessage(Message message) {
            if (message.what == mFakeR.getId("id","zxing_prewiew_size_ready")) {
                previewSized((Size) message.obj);
                return true;
            } else if (message.what == mFakeR.getId("id","zxing_camera_error")) {
    ...
    protected void initializeAttributes(AttributeSet attrs) {
        TypedArray styledAttributes = getContext().obtainStyledAttributes(attrs, new int[]{mFakeR.getId("styleable", "zxing_camera_preview")});

        int framingRectWidth = (int) styledAttributes.getDimension(mFakeR.getId("styleable","zxing_camera_preview_zxing_framing_rect_width"), -1);
        int framingRectHeight = (int) styledAttributes.getDimension(mFakeR.getId("styleable","zxing_camera_preview_zxing_framing_rect_height"), -1);

        if (framingRectWidth > 0 && framingRectHeight > 0) {
            this.framingRectSize = new Size(framingRectWidth, framingRectHeight);
        }

        this.useTextureView = styledAttributes.getBoolean(mFakeR.getId("styleable","zxing_camera_preview_zxing_use_texture_view"), true);

        // See zxing_attrs.xml for the enum values
        int scalingStrategyNumber = styledAttributes.getInteger(mFakeR.getId("styleable","zxing_camera_preview_zxing_preview_scaling_strategy"), -1);
